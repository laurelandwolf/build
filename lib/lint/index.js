...
module.exports = function startLint (options) {
  ...
  function lintFiles () {
    ...
    getFilesToLint(function (err, fileObject) {
        ...
        _.each(report.results, function (result) {
          ...
          _.each(result.messages, function (message) {
            ...
            if (message.severity === 2) {
              var severity = message.severity < 2 ? format.yellow(logSymbols.warning + ': ') : format.red(logSymbols.error + ': ');
      ...
      if (!hasErrors) {
        console.log(format.green(logSymbols.success + ' No Linting Errors'));
        console.log('');
